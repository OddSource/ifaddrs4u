...
class TestInterfaceBrowser(TestCase):
    def test_for_each_then_iterate(self):
        counts = [0, 0]
    ...
    def test_iterate_then_foreach(self):
        counts = [0, 0]
    ...
    def test_iterate_then_iterate(self):
        counts = [0, 0]
    ...
    def test_for_each_with_returning_false(self):
        counts = [0, 0]
    ...
    def test_get_by_index(self):
        ...
        if IS_MACOS:
            lo = browser.get_interface(1)
            self.assertEqual(lo.index, 1)
            self.assertIsNotNone(lo.name)

            lo = browser[1]
            self.assertEqual(lo.index, 1)
            self.assertIsNotNone(lo.name)

            en = browser.get_interface(2)
            self.assertEqual(en.index, 2)
            self.assertIsNotNone(en.name)
